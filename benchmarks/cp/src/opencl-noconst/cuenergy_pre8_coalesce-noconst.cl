...
__kernel void cenergy(int numatoms, float gridspacing, __global float * energygrid, __global float4 *atominfo) {
  ...
  for (atomid=0; atomid<numatoms; atomid++) {
    ...
    energyvalx1 += atominfo[atomid].w * (1.0f / sqrt(dx1*dx1 + dyz2));
    energyvalx2 += atominfo[atomid].w * (1.0f / sqrt(dx2*dx2 + dyz2));
    energyvalx3 += atominfo[atomid].w * (1.0f / sqrt(dx3*dx3 + dyz2));
    energyvalx4 += atominfo[atomid].w * (1.0f / sqrt(dx4*dx4 + dyz2));
    energyvalx5 += atominfo[atomid].w * (1.0f / sqrt(dx5*dx5 + dyz2));
    energyvalx6 += atominfo[atomid].w * (1.0f / sqrt(dx6*dx6 + dyz2));
    energyvalx7 += atominfo[atomid].w * (1.0f / sqrt(dx7*dx7 + dyz2));
    energyvalx8 += atominfo[atomid].w * (1.0f / sqrt(dx8*dx8 + dyz2));
  }
