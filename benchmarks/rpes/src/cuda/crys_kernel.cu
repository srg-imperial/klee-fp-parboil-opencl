...
__device__ void GetBases(int, int, int, int&, int&, int&, int&, int&, int);
...
__global__  void ComputeX(uint4* d_Work, float* d_Output, int Offset, float4* d_Coors,
              float2 *d_Sprms)
{
  int blid = blockIdx.x;
  ...
  int blockSizeLog = (int)log2f(blockSize);
  ...
  if(localthid < Maxthid)
    {
      ...
      float preexp = prod12 / sum12 * R12 + 
	             prod34 / sum34 * R34;
      float preintegral = Param1.y * Param2.y * Param3.y * 
			  Param4.y * __expf(- preexp) / 
			  (sum12 * sum34) * (1 / sqrtf(sum12 + sum34));
      ...
      if(preintegral * preintegral > 1.0e-23f)
	{
	  ...
	  float rho = sum12 * sum34 / (sum12 + sum34);
...
__global__  void DoReduction(float* d_ReductionSum, float* d_Output, 
			    uint2* d_FinalReduce, int block_size, int Offset)
{
  ...
  if(thid < offset)
    Data[thid] = d_Output[firstElement + thid];
  else
    Data[thid] = 0.0f;
  __syncthreads();
  ...
  if(!thid)
    d_ReductionSum[blid + Offset] = Data[0];
}
...
void GetBases(int Work, int thid, int Shift, int& localthid, 
	      int& a1, int& a2, int& a3, int& a4, int blockSizeLog)
{
