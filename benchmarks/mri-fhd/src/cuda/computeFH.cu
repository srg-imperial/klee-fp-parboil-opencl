...
#ifdef PROFILING
cudaEvent_t evRhoPhiStart, evRhoPhiStop;
cudaEvent_t *evFHStart, *evFHStop;
int numFH;
#endif
...
void computeRhoPhi_GPU(int argc, char ** argv, int numK, 
                       float* phiR_d, float* phiI_d, float* dR_d, float* dI_d,
                       float* realRhoPhi_d, float* imagRhoPhi_d)
{
  int nThreadsPerBlock = KERNEL_RHO_PHI_THREADS_PER_BLOCK;
  getCmdLineParamInt("-rhophi-local", argc, argv, (int*)&nThreadsPerBlock);

  int rhoPhiBlocks = numK / nThreadsPerBlock;
  if (numK % nThreadsPerBlock)
    rhoPhiBlocks++;

  dim3 DimRhoPhiBlock(nThreadsPerBlock, 1);
...
#ifdef PROFILING
  cudaEventCreate(&evRhoPhiStart);
  cudaEventCreate(&evRhoPhiStop);
  cudaEventRecord(evRhoPhiStart, 0);
#endif
...
#ifdef PROFILING
  cudaEventRecord(evRhoPhiStop, 0);
#endif

}

void computeFH_GPU(int argc, char ** argv, int numK, int numX, 
                   float* x_d, float* y_d, float* z_d,
                   kValues* kVals,
                   float* outR_d, float* outI_d)
{
  int nThreadsPerBlock = KERNEL_FH_THREADS_PER_BLOCK;
  getCmdLineParamInt("-fh-local", argc, argv, (int*)&nThreadsPerBlock);

  int FHGrids = numK / KERNEL_FH_K_ELEMS_PER_GRID;
  if (numK % KERNEL_FH_K_ELEMS_PER_GRID)
    FHGrids++;
  int FHBlocks = numX / nThreadsPerBlock;
  if (numX % nThreadsPerBlock)
    FHBlocks++;
  dim3 DimFHBlock(nThreadsPerBlock, 1);
...
#ifdef PROFILING
  numFH = FHGrids;
  evFHStart = (cudaEvent_t*) malloc (sizeof(cudaEvent_t) * numFH);
  evFHStop  = (cudaEvent_t*) malloc (sizeof(cudaEvent_t) * numFH);
  for (int i=0; i<numFH; i++)
  {
    cudaEventCreate(&evFHStart[i]);
    cudaEventCreate(&evFHStop[i]);
  }
#endif
...
#ifdef PROFILING
    cudaEventRecord(evFHStart[FHGrid], 0);
#endif
...
#ifdef PROFILING
    cudaEventRecord(evFHStop[FHGrid], 0);
#endif
  }
}
